...
# Default-Stop:      0 1 6
# Short-Description: Start the RTDB process
### END INIT INFO

# If you modify this, please make sure to also edit rtdb.service

# Environment variables can be set in /etc/default/rtdbv2. These will override
# any corresponding config file values.
DEFAULT=/etc/default/rtdbv2
...
function start() {
    ...
    if command -v start-stop-daemon &>/dev/null; then
        start-stop-daemon \
            ...
            --exec $DAEMON \
            -- \
            $RTDB_OPTS >>$STDOUT 2>>$STDERR &
    else
        local CMD="$DAEMON $RTDB_OPTS >>$STDOUT 2>>$STDERR &"
        su -s /bin/sh -c "$CMD" $USER
    fi

    # Sleep to verify process is still up
    sleep 1
    echo $(pgrep -u $USER -f rtdb) > $PIDFILE
    if [ -f $PIDFILE ]; then
        # PIDFILE exists
